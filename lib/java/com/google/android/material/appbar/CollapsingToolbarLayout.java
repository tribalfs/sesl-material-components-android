...
public class CollapsingToolbarLayout extends FrameLayout {
  ...
  private HashMap<AppBarModel<AppBarView>, ViewGroup> mSuggestViewHashMap = new HashMap<>();
  //sesl
  ...
  public CollapsingToolbarLayout(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    scrimAnimationFadeInInterpolator = MotionUtils.resolveThemeInterpolator(context, R.attr.motionEasingStandardInterpolator, AnimationUtils.FAST_OUT_LINEAR_IN_INTERPOLATOR);
    scrimAnimationFadeOutInterpolator = MotionUtils.resolveThemeInterpolator(context, R.attr.motionEasingStandardInterpolator, AnimationUtils.LINEAR_OUT_SLOW_IN_INTERPOLATOR);

    setContentScrim(a.getDrawable(R.styleable.CollapsingToolbarLayout_contentScrim));
    setStatusBarScrim(a.getDrawable(R.styleable.CollapsingToolbarLayout_statusBarScrim));

    toolbarId = a.getResourceId(R.styleable.CollapsingToolbarLayout_toolbarId, -1);
    forceApplySystemWindowInsetTop = a.getBoolean(R.styleable.CollapsingToolbarLayout_forceApplySystemWindowInsetTop, false);//sesl
    a.recycle();
  ...
  protected void onConfigurationChanged(Configuration newConfig) {
    ...
    mHeightProportion = ResourcesCompat.getFloat(getResources(), R.dimen.sesl_appbar_height_proportion);
    updateDefaultHeight();
    updateTitleLayout();
    //sesl
  }
  ...
  public CharSequence getTitle() {
    return collapsingTitleEnabled ? collapsingTextHelper.getText() : mExtendedTitle.getText()/*sesl*/;
  }
   ...
   *
   * <p>The title displayed by this view will scroll and fade based on the scroll offset.
   * Setting this to false will also disable the fading effect of the collapsed title.
   *
...
//      collapsingTextHelper.setExpansionFraction(Math.abs(verticalOffset) / (float) expandRange);
  ...
  private void updateTitleLayout() {
    ...
    if (mTitleEnabled) {
      ...
      if (Math.abs(mHeightProportion - LAND_HEIGHT_PERCENT) >= EPSILON) {
        mExtendedTitle.setSingleLine(false);
        mExtendedTitle.setMaxLines(2);
      } else if (mSubTitleEnabled) {
        ...
        if (maxLines > 1) {
          ...
          try {
            ...
            if (mSubTitleEnabled && statusBarHeight > 0) {
              statusBarHeight += resources.getDimensionPixelSize(androidx.appcompat.R.dimen.sesl_action_bar_top_padding);
            }
  ...
  public void seslSetSuggestView(@Nullable AppBarModel<AppBarView> appBarModel) {
    for (AppBarModel<AppBarView> appBarViewAppBarModel : mSuggestViewHashMap.keySet()) {
    ...
    if (getParent() instanceof AppBarLayout) {
      ((AppBarLayout) getParent()).seslSetSuggestion(!mSuggestViewHashMap.isEmpty());
    }
  }
  //sesl7
